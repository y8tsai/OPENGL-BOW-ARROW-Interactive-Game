...
vec4 mat4::operator*(const vec4 &b) const {
	vec4 r;
	r.v[0] = m[0][0]*r.v[0] + m[0][1]*r.v[1] + m[0][2]*r.v[2] + m[0][3]*r.v[3];
    r.v[1] = m[1][0]*r.v[0] + m[1][1]*r.v[1] + m[1][2]*r.v[2] + m[1][3]*r.v[3];
    r.v[2] = m[2][0]*r.v[0] + m[2][1]*r.v[1] + m[2][2]*r.v[2] + m[2][3]*r.v[3];
    r.v[3] = m[3][0]*r.v[0] + m[3][1]*r.v[1] + m[3][2]*r.v[2] + m[3][3]*r.v[3];
	return r;
}
...
quat quat::rotate(float rad, float x, float y, float z) {
	quat rotated; 
	rotated.x = x * sinf(rad/2);
	rotated.y = y * sinf(rad/2);
	rotated.z = z * sinf(rad/2);
