...
void Core::OnKeyDown(SDL_Keycode sym, Uint16 mod) {
	...
	float velocity = 0.3f; //move 0.3 units per *frame
...
void Core::OnMouseMove(int mX, int mY, int relX, int relY, bool lt, bool rt, bool mid) {
	...
	quat yrot = quat();
	yrot = yrot.rotate( -relX * DEG2RAD(0.3f), 0.0f, 1.0f, 0.0f );
	quat ytarget;
	...
	quat xrot = quat();
	xrot = xrot.rotate( relY * DEG2RAD(mouseYSensitivity), axis.v[0], axis.v[1], axis.v[2] );
	quat xtarget;
